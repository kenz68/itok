...
class FirebaseClient {
    ...
    func signIn(completion: @escaping (Firebase.User?, Error?) -> Void) {
        ...
        Auth.auth().signInAnonymously(completion: { (user, error) in
            if let user = user {
                self.dataReference.child("learners/\(user.user.uid)").observeSingleEvent(of: .value, with: { snapshot in
                    if let value = snapshot.value as? NSObject, value is NSNull {
                        self.dataReference.child("learners/\(user.user.uid)").updateChildValues(["name": User.current.name])
                    }
                })
            }
    ...
    func signIn(email: String, password: String, completion: @escaping (Firebase.User?, Error?) -> Void) {
        Auth.auth().signIn(withEmail: email, password: password, completion: { (user, error) in
            if let user = user {
                User.current.uid = user.user.uid
